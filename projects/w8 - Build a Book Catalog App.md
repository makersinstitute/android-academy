...
From [here](https://github.com/makersinstitute/BookCatalog-v1), choose `master` branch.

####. Let's take a look at the code, there are three `Java Class`, and two `XML Layout`
	- `MainActivity.class`
...
4. Next, we declared another class, as we called in previous code Snippet. Here is the code to request the data from API server.
	...
	2. After we have the return value from `doInBackground` which is a `ArrayList<Book>`, we process it inside the `onPostExecute`. We override it then update the `ListView` to show the response data. Before that, *dismiss the loading view*. Make sure to check the `bookList`, if its empty, then update `ListView` with empty data, if not, show the `bookList`

			```java
            @Override
            protected void onPostExecute(ArrayList < Book > bookList) {

                // dismiss loading view here
...
8. As listed in `doInBackground`, we have `extractBookInfoFromJson(String bookJSON)`, so we declare anything inside it here,
	...
	3.  Next, we try to parse the String input of this method, the `bookJSON` inside a `try/catch`. Now we `try` to parse `bookJSON`
		...
		1. Cast it into `JSONObject` and check  the `"totalItems"` key inside that `JSON`. If the item is zero, then there is no book matched our keyword.
		...
		5. Inside the `catch`, we catch the possible error
		...
		6. Return the `books` since this method required you to return an `ArrayList<Book>` and the `books`'s type is `ArrayList<Book>`
